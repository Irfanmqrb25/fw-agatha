...
export function DoaLingkunganContent({
  ...
  const [kepalaKeluargaState, setKepalaKeluargaState] =
    useState<KeluargaForSelect[]>(initialKeluarga);
  const [riwayatState, setRiwayatState] =
    useState<{ nama: string; totalHadir: number; persentase: number }[]>(
      initialRiwayat
    );
  const [rekapitulasiState, setRekapitulasiState] =
    useState<
      { bulan: string; jumlahKegiatan: number; rataRataHadir: number }[]
    >(initialRekapitulasi);
  ...
  // Editing states
  const [editingJadwal, setEditingJadwal] = useState<JadwalDoling | undefined>(
    undefined
  );
  const [editingDetil, setEditingDetil] = useState<DetilDoling | undefined>(
    undefined
  );
  const [editingAbsensi, setEditingAbsensi] = useState<
    AbsensiDoling | undefined
  >(undefined);
    ...
    try {
      const currentYear =
        selectedTahun === "all"
          ...
          : parseInt(selectedTahun);

      // Ambil semua data secara parallel untuk kinerja yang lebih baik
      const [dolingData, keluargaData, riwayatData, rekapitulasi] =
        await Promise.all([
          getAllDoling(),
          getKeluargaForSelection(),
          getRiwayatKehadiran(),
  ...
  useEffect(() => {
    if (
      !initialDoling.length ||
      !initialRiwayat.length ||
      !initialRekapitulasi.length ||
      !initialKeluarga.length
    ) {
      fetchData();
    } else {
      setLoading(false);
  ...
  const handleResetKepalaKeluarga = async () => {
    try {
      ...
      toast.success(
        "Reset berhasil. Semua kepala keluarga dapat dipilih kembali."
      );
    } catch (error) {
  ...
  const handleAddAbsensi = async () => {
    ...
    const dolingIdToUse =
      selectedDolingId || (jadwalState.length > 0 ? jadwalState[0].id : null);

    if (dolingIdToUse) {
      try {
        // Dapatkan daftar keluarga dengan filter yang sudah terabsensi
        const keluargaFilteredData = await getKeluargaForSelection(
          dolingIdToUse
        );
    ...
    <div className="space-y-6">
      ...
      >
        <TabsList className="">
          ...
          <TabsTrigger
            value="absensi-doling"
            className="text-xs sm:text-sm py-2"
          >
            Absensi
          </TabsTrigger>
          <TabsTrigger value="detil-doling" className="text-xs sm:text-sm py-2">
            Detil
          </TabsTrigger>
          <TabsTrigger
            value="riwayat-doling"
            className="text-xs sm:text-sm py-2"
          >
            Riwayat
          </TabsTrigger>
      ...
      <Dialog open={resetKKConfirmOpen} onOpenChange={setResetKKConfirmOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Anda yakin ingin melakukan reset data Kepala Keluarga? Tindakan
              ini akan memastikan semua Kepala Keluarga (termasuk yang sudah
              pernah menjadi tuan rumah) dapat dipilih kembali sebagai tuan
              rumah dalam jadwal doa lingkungan.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="mt-4">
            <Button
              variant="outline"
              onClick={() => setResetKKConfirmOpen(false)}
            >
              Batal
            </Button>
            <Button onClick={handleResetKepalaKeluarga}>Ya, Reset</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
