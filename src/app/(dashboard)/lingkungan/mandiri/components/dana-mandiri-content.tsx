...
export default function DanaMandiriContent() {
  ...
  const canModifyData = userRole
    ? ["SUPER_USER", "BENDAHARA"].includes(userRole)
    : false;
  ...
  const [editingTransaction, setEditingTransaction] =
    useState<DanaMandiriTransaction | null>(null);
  ...
  const [slipPenyetoranDialogOpen, setSlipPenyetoranDialogOpen] =
    useState(false);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (transactionsResult.success && transactionsResult.data) {
        ...
        toast.error(
          transactionsResult.error || "Gagal mengambil data transaksi"
        );
      }
  ...
  const handleTransactionSelectChange = (
  ...
  const handleAddTransaction = async (values: TransactionFormValues) => {
    ...
    try {
      ...
      if (values.periodeBayar)
        formData.append("periodeBayar", values.periodeBayar.toString());
  ...
  const handleSetDues = async (values: SetDuesValues) => {
    ...
    try {
      ...
      if (result.success) {
        toast.success(
          `Iuran Dana Mandiri tahun ${
            values.year
          } berhasil ditetapkan sebesar ${formatCurrency(values.amount)}`
        );
  ...
  const handleSubmitToParoki = async (values: {
    ...
    try {
      ...
      if (values.submissionNote)
        formData.append("submissionNote", values.submissionNote);
  ...
  const handleSendReminder = async (values: SendReminderValues) => {
    ...
    try {
      ...
      if (result.success) {
        toast.success(
          `Berhasil mengirim pengingat kepada ${
            result.count || values.familyHeadIds.length
          } kepala keluarga`
        );
    ...
    <div className="space-y-4">
      ...
      <Tabs defaultValue="transactions" className="w-full">
          ...
          <div className="flex flex-wrap gap-2">
            {(canModifyData || userRole === "SUPER_USER") && (
              <Button
                ...
                title={
                  !isInitialBalanceSet
                    ? "Saldo awal kas lingkungan harus diset terlebih dahulu"
                    : "Input Transaksi"
                }
              >
                Input Transaksi
        ...
        <TabsContent value="monitoring" className="space-y-4">
          <div className="flex flex-wrap gap-2">
            {(canModifyData || userRole === "SUPER_USER") && (
              <Button onClick={handleOpenSetDuesDialog} disabled={isMutating}>
      ...
      <ReminderDialog
        ...
        familyList={arrears
          .filter((a) => selectedFamilyIds.includes(a.keluargaId))
          .map((a) => ({
            id: a.keluargaId,
